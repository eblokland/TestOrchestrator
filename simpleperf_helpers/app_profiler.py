...
class ProfilerBase(object):
    ...
    def start_profiling(self, target_args):
        ...
        args += target_args
        adb_args = [self.adb.adb_path, 'shell', '-x'] + args
        logging.info('run adb cmd: %s' % adb_args)
        print(adb_args)
        self.record_subproc = subprocess.Popen(adb_args, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    ...
    def stop_profiling(self) -> List[str]:
        ...
        while True:
            ...
            time.sleep(1)
        return self.record_subproc.stdout.readlines()
